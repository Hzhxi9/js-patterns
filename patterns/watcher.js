 ...
 *      - 时间上的解耦
 *      - 对象之间的解耦
 *      - 应用广泛, 可以用在异步编程, 也可以帮助我们完成更松耦合的代码编写
 *      - 可以用来帮助实现一些别的设计模式, 比如中介者模式
 *      - 从架构上来看, 无论是 MVC 还是 MVVM, 都少不了发布订阅的参与, 而且 Javascript 本身也是一门基于事件驱动的语言
 * 
 *    缺点:
 *      
 *      - 创建订阅者本身需要消耗一些时间和内存
 *      - 当你订阅一个消息后, 也许这个消息最后都未发生, 但这个订阅者会始终存在于内存中
 *      - 发布订阅模式虽然可以弱化对象之间的联系, 但如果过度使用, 对象和对象之间的必要联系也会深埋在背后, 会导致程序难以跟踪维护和理解
 *        特别是有多个发布者和订阅者嵌套一起的时候, 要跟踪一个 bug 不是一件轻松的事情
 *
 *    必须先订阅再发布吗:
 * 
 ...
 * 
 *    - 用 Javascript 实现发布订阅的便利性
 *      
 *      在 Javascript 中, 我们用注册回调函数的形式来替代传统的发布订阅模式, 显得优雅和简单
 *      
 *      另外我们无需选择使用推模型和拉模型 
 * 
 *        - 推模型: 指在事件发生时, 发布者一次性把所有更改的状态和数据都发送给订阅者
 *        - 拉模型: 发布者仅仅通知订阅者事件已经发生了, 此外发布者要提供一些公开的接口供订阅者来主动拉取数据
 *                 好处： 可以让订阅者按需获取
 *                 坏处: 有可能让发布者变成一个门户大开的对象, 同时增加了代码的代码量和复杂度
 * 
 *      在 Javascript 中, arguments 可以很方便地表示参数列表, 所以我们一般选择推模型, 使用 Function.prototype.apply 方法把所有参数都推送给订阅者 
 */
